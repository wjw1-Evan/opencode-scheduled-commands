...
任务没跑？先开调试日志，看调度器每次 tick 的决策：

```bash
SCHEDULED_COMMANDS_DEBUG=1 opencode
```

调试模式（`level: debug`）会输出：

- **每次 tick**：加载的任务数、正在运行的任务、每个任务的下次触发时间与跳过原因（未到期 / 仍在运行 / 未配置调度 / `enabled: false`）
- **锁细节**：锁文件路径、获取锁的进程 PID
- **缺失配置**：`schedules.json` 不存在时给出明确的创建提示

常见问题：

- **锁文件残留导致"调度器不启动"**：调度器通过 `.opencode/.scheduled-lock`（内容为持有者 PID）防止同一目录多实例。若进程崩溃/被杀（Docker 重启、Desktop 重启等）锁残留，插件会**自动检测持有者 PID 是否存活并恢复**，无需手动处理；若日志显示 "Scheduler already running ... held by PID x" 且该进程确实还活着，请删掉该锁文件并重启 opencode
- **目录不可写**：插件需要在项目目录创建锁文件与状态文件；只读目录（沙箱/只读挂载）会导致启动失败，日志会明确提示 "Failed to start scheduler" 并附目录权限检查建议
- **看不到日志**：错误与警告会同时输出到控制台（stderr）与 opencode 日志服务；即使 SDK 日志通道不可用也不会静默
